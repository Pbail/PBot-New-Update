...
let channelId = '-1002086504362'; // fsubs Id
        ...
        try {
            ...
            if (admins.includes(String(ctx.message.from.id)) && ctx.message.chat.type === 'private') {
                ...
                [{
                        ...
                        {
                            text: fsubsButtonText,
                            callback_data: 'fsubsonoff'
                        }
                    ],
                    [{
                        text: '❌ Close',
    ...
    bot.on('callback_query', async (ctx) => {
        ...
        if (action === 'about') {
            ...
            try {
                ...
                // Determine button text based on settings
                let shortLinkButtonText, fsubButtonText, fsubsButtonText;
            ...
            } else if (action === 'fsubs') {
            ctx.answerCbQuery(`This is button For Fsubs On Off`, {
                show_alert: true
            })
        } else {
...
async function initializingBot() {
    try {
        ...
        if (!existingEntry) {
            ...
            console.log('BotSetting created with shortLink/Fsub/Fsubs set to ON.');
        }
